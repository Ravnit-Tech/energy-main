...
const DEPOT_NAME = "Lagos Main Depot";

function LagosMainDepot() {
  ...
  return (
    <>
      <Head><title>Lagos Main Depot | e-Nergy</title></Head>
                ...
                return (
                  <button
                    ...
                    className={`px-4 md:px-6 py-2 md:py-2.5 rounded-lg font-bold text-xs md:text-sm transition-all duration-200 shadow-md
                      ${
                        activeProduct === item
                          ? `${colors[item].active} text-white scale-105`
                          : `bg-white/90 text-gray-700 ${colors[item].hover}`
                      }`}
                  >
                    {item}
                  </button>
        ...
        <div className="w-full lg:w-[500px] xl:w-[600px] flex items-center justify-center">
          <div className="w-full bg-white/95 backdrop-blur-sm rounded-lg shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-green-600 to-orange-400 px-4 md:px-6 py-3 md:py-4">
              <h2 className="text-center font-bold text-lg md:text-2xl text-white">
                ENERGY OIL PRODUCTS
              </h2>
              <p className="text-center text-white text-xs md:text-sm mt-1 opacity-90">
                Lagos Main Depot
              </p>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-gray-300">
                      <th className="px-2 md:px-4 py-2 md:py-3 text-left font-bold text-gray-700 text-xs md:text-base">
                        Product
                      </th>
                      <th className="px-2 md:px-4 py-2 md:py-3 text-left font-bold text-gray-700 text-xs md:text-base">
                        Status
                  ...
                  <tbody>
                    {Object.entries(products).map(([key, data]) => (
                      ...
                      >
                        <td className="px-2 md:px-4 py-3 md:py-4 font-bold text-gray-800 text-xs md:text-base">
                          {key}
                        </td>
                        <td
                          className={`px-2 md:px-4 py-3 md:py-4 font-bold text-xs md:text-base ${
                        ...
                        <td className="px-2 md:px-4 py-3 md:py-4 text-gray-600 font-semibold text-xs md:text-base">
                          {data.price}
                        </td>
