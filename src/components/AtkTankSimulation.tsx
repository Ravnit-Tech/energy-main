...
const AtkTankSimulation = ({ level, logo }: AtkTankSimulationProps) => {
  ...
  // Dynamically compute calibration marks based on maxVolume
  // Tank SVG: calibration bottom y=745, top y=130, total height=615px
  const CALIBRATION_BOTTOM = 745;
  const CALIBRATION_TOP = 130;
  const TANK_HEIGHT_PX = CALIBRATION_BOTTOM - CALIBRATION_TOP; // 615

  // Determine a nice major interval (aim for ~10 major ticks)
      ...
      <div className="bg-white/90 backdrop-blur rounded-lg p-2 md:p-3 shadow-lg border border-slate-200 mb-2 md:mb-3">
        <div className="grid grid-cols-3 gap-1 md:gap-2 text-center">
          ...
          <div>
            ...
            <p className="text-sm md:text-lg font-bold bg-gradient-to-r from-green-600 to-green-800 bg-clip-text text-transparent">
      ...
      <div className="flex-1 flex items-center justify-center overflow-hidden">
        <div className="w-full h-[110%]">
          <svg viewBox="0 0 1200 850" className="w-full h-full"
            ...
            <defs>
              <linearGradient id="steel-atk" x1="0%" x2="100%">
              ...
              <linearGradient id="atk-liquid" x1="0%" x2="100%">
              ...
              <radialGradient id="depth-atk">
                <stop offset="0%" stopColor="#bbf7d0" stopOpacity="0.4"/>
                <stop offset="100%" stopColor="#166534" stopOpacity="0"/>
              </radialGradient>
              <pattern id="wave-atk" x="0" y="0" width="120" height="25" patternUnits="userSpaceOnUse">
                <path d="M0,12 Q30,8 60,12 T120,12 V25 H0 Z" fill="rgba(187,247,208,0.2)">
                  <animateTransform attributeName="transform" type="translate" from="0 0" to="120 0" dur="4s" repeatCount="indefinite"/>
                </path>
              </pattern>
              <linearGradient id="glass-atk" x1="0%" x2="100%">
              ...
              <linearGradient id="blue-atk" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor="rgba(59,130,246,0.8)"/>
                <stop offset="100%" stopColor="rgba(59,130,246,0)"/>
              </linearGradient>
              <linearGradient id="orange-atk" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor="rgba(249,115,22,0.8)"/>
                <stop offset="100%" stopColor="rgba(249,115,22,0)"/>
              </linearGradient>
              <linearGradient id="concrete-atk" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor="#9ca3af"/><stop offset="50%" stopColor="#d1d5db"/>
                <stop offset="100%" stopColor="#6b7280"/>
              </linearGradient>
              <pattern id="rust-atk" width="50" height="50" patternUnits="userSpaceOnUse">
                <circle cx="10" cy="10" r="1" fill="#92400e" opacity="0.3"/>
                <circle cx="35" cy="25" r="1.5" fill="#a16207" opacity="0.25"/>
              </pattern>
              <pattern id="concreteTexture-atk" width="60" height="60" patternUnits="userSpaceOnUse">
            ...
            <path d="M 80 745 Q 80 755, 85 770 L 1115 770 Q 1120 755, 1120 745 Z" fill="url(#concrete-atk)" stroke="#4b5563" strokeWidth="2"/>
            <path d="M 80 745 Q 80 755, 85 770 L 1115 770 Q 1120 755, 1120 745 Z" fill="url(#concreteTexture-atk)" opacity="0.4"/>
            <ellipse cx="600" cy="755" rx="520" ry="15" fill="#71717a" stroke="#52525b" strokeWidth="1.5"/>
            <ellipse cx="600" cy="745" rx="520" ry="13" fill="url(#concrete-atk)" stroke="#6b7280" strokeWidth="2"/>
            <ellipse cx="600" cy="744" rx="520" ry="13" fill="url(#concreteTexture-atk)" opacity="0.5"/>
            ...
            {/* Conical bottom */}
            <path d="M 90 650 Q 90 715, 122 738 L 1078 738 Q 1110 715, 1110 650 Z" fill="url(#steel-atk)" stroke="#475569" strokeWidth="3"/>
            {[...Array(40)].map((_,i) => {
              const y = 665+i*3.5;
            ...
            <path d="M 90 130 L 90 650 Q 90 715, 122 738 L 1078 738 Q 1110 715, 1110 650 L 1110 130 Z" fill="url(#glass-atk)" stroke="#64748b" strokeWidth="4"/>
            <path d="M 90 130 L 90 650 Q 90 715, 122 738 L 1078 738 Q 1110 715, 1110 650 L 1110 130 Z" fill="url(#rust-atk)" opacity="0.15"/>
            ...
            {[220,340,460,580].map((y,i) => (
              <g key={`b-${i}`}>
                <rect x="90" y={y} width="1020" height="12" fill="url(#steel-atk)" opacity="0.6" stroke="#475569"/>
                {[...Array(42)].map((_,j) => <circle key={`br2-${j}`} cx={100+j*24} cy={y+6} r="2" fill="#1e293b" stroke="#334155" strokeWidth="0.5"/>)}
              </g>
            ))}

            {/* Liquid */}
            <g clipPath="url(#clip-atk)">
              <path d={`M 90 ${650-liquidLevel*5.2} L 90 650 Q 90 715, 122 738 L 1078 738 Q 1110 715, 1110 650 L 1110 ${650-liquidLevel*5.2} Z`} fill="url(#atk-liquid)"/>
              <ellipse cx="600" cy={650-liquidLevel*5.2+100} rx="470" ry={liquidLevel*2.6} fill="url(#depth-atk)"/>
              <rect x="90" y={650-liquidLevel*5.2-10} width="1020" height="50" fill="url(#wave-atk)"/>
              <ellipse cx="600" cy={650-liquidLevel*5.2+8} rx="485" ry="12" fill="rgba(187,247,208,0.3)"/>
              {[...Array(18)].map((_,i) => {
                const xPos = 155+i*52; const size = 3+(i%4);
                return (
                  <g key={`bub-${i}`}>
                    <circle cx={xPos} cy={640-liquidLevel*5} r={size} fill="rgba(187,247,208,0.6)" stroke="rgba(34,197,94,0.3)">
            ...
            {/* Lighting */}
            <rect x="90" y="130" width="104" height="520" fill="url(#blue-atk)" opacity="0.6"/>
            <rect x="1006" y="130" width="104" height="520" fill="url(#orange-atk)" opacity="0.6"/>
            ...
            {logo && (
              <g>
                ...
                <image href={logo} x="517" y="307" width="166" height="166" clipPath="url(#logo-clip-atk)" preserveAspectRatio="xMidYMid meet" style={{ filter: 'drop-shadow(0 2px 10px rgba(0,0,0,0.5))' }}/>
              </g>
            )}

            {/* Calibration — dynamically generated from maxVolume */}
            {calibrationMarks.map((m) => {
              if (m.y < CALIBRATION_TOP || m.y > CALIBRATION_BOTTOM) return null;
              return (
                <g key={`c-${m.i}`}>
                  {m.isMajor ? (
                    <>
...
export default AtkTankSimulation;
